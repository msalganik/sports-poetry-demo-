...
- **Python (`orchestrator.py`)**:
  - Reads `config.json`
  - Creates session directory `output/{session_id}/`
  - Launches subagents (via subprocess/API)
  - Manages parallel execution
  - Captures agent outputs
  - Writes `output/{session_id}/execution_log.jsonl` (detailed provenance)
  - Writes `output/{session_id}/usage_log.jsonl` (aggregate analytics)
...
├── SPEC.md                      # This file
└── output/
    ├── {session_id}/            # Timestamped session directory
    │   ├── basketball/
    │   │   ├── haiku.txt
    │   │   ├── sonnet.txt
    ...
    │   └── usage_log.jsonl      # Aggregate analytics (this run)
...
2. ✅ Session directory `output/{session_id}/` with symlink `output/latest`
3. ✅ One directory per sport in `output/{session_id}/`
4. ✅ `haiku.txt` and `sonnet.txt` for each sport (or logged failure)
5. ✅ `metadata.json` for each sport
6. ✅ `analysis_report.md` with comparative analysis in session directory
7. ✅ `execution_log.jsonl` with complete provenance in session directory
8. ✅ `usage_log.jsonl` with aggregate entry in session directory
9. ✅ All errors logged, workflow completes gracefully
